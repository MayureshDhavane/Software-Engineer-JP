...
public class TransactionValidator {
    ...
    public List<String> findInsufficientFundAccounts(List<Account> accounts,
        ...
        for (TransferRequest transfer : transfers) {
            Account fromAccount = findAccount(accounts, transfer.getFromAccountId());
            Account toAccount = findAccount(accounts, transfer.getToAccountId());

            if (fromAccount == null || toAccount == null) {
                continue; // Skip invalid transfers
            }

            // Simulate the transfer directly on the original accounts
